...
function Todos () {
  ...
  const getTodoList = async () => {
    ...
    setTodos(todoList)
    setUndoneTodoCounts(
      await todoList.filter(todo => todo.status === false).length
    )
  ...
  const getTodosByStatus = async status => {
    ...
    const todoList = await fetchTodos()
    setTodos(todoList)
    const undoneTodolist = await todoList.filter(todo => {
  ...
  const handleInputChange = e => {
    setNewTodo({ content: e.target.value.trim() })
  }
  ...
  return (
    <div className='container mt-5'>
      <div className='mx-auto' style={{ width: '500px' }}>
        ...
        <form
          className='d-flex full justify-content-center'
          ...
          >
            <ul className='d-flex w-100 p-0'>
              <li
                className='btn'
                key='all'
                style={todoListType === 'all' ? activeStyle : inactiveStyle}
                onClick={() => {
                  getTodoList()
                }}
              >
              ...
              <li
                className='btn'
                key='undone'
                style={todoListType === 'undone' ? activeStyle : inactiveStyle}
                onClick={() => {
                  getUndoneTodos()
                }}
              >
              ...
              <li
                className='btn'
                key='done'
                style={todoListType === 'done' ? activeStyle : inactiveStyle}
                onClick={() => {
                  getDoneTodos()
                }}
              >
                      ...
                      >
                        ...
                        <div>
                          {isEditing === todo.id ? (
                            <>
                              <button
                                className='btn ms-2 fw-bold mt-1 text-danger'
                                onClick={() => editTodo(todo.id, editText)}
                              >
                                更改
                              </button>
                              <button
                                className='btn ms-2 fw-bold mt-1 text-secondary'
                            ...
                            <>
                              <button
                                className='btn ms-2 fw-bold mt-1 text-success'
                                onClick={() => {
